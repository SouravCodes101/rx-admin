...
class ViewVendorCmp extends React.Component {
    async componentDidMount() {
        ...
        if(respObj===false) {
            ...
            this.setState({id:respObj._id,isActive:isActive,name:respObj.orgName,email:respObj.orgEmail,phone:respObj.orgPhone,addressLine1:respObj.addressLine1,addressLine2:respObj.addressLine2,area:respObj.area,pinCode:respObj.pinCode,landmark:respObj.landmark,city:respObj.city,state:respObj.state,country:respObj.country});
        }
    }
    ...
    updateOrg=async ()=>{
        ...
        let respObj=await VendorService.editVendor(payload);
        this.setState({isLoading:false});
        if(respObj===false) {
            toast.error(process.env.ERROR_MSG);
        }else {
            ...
            <React.Fragment>
                <div className={"padding-container"}>
                    <Breadcrumb>
                        ...
                        <Breadcrumb.Item>View/Edit Vendor</Breadcrumb.Item>
                        <Breadcrumb.Item active>{this.state.name}</Breadcrumb.Item>
                    </Breadcrumb>

                    <FlexboxGrid>
                        <FlexboxGrid.Item colspan={5}>
                            <CommonMenuCmp vendorId={this.props.vendorId} activeKey={"1"}/>
                        </FlexboxGrid.Item>
                                    ...
                                    <Row>
                                        <Col xs={8}>
                                            <Form.Group controlId="addAddress" className={"margin-btn"}>
                                                <Button appearance="primary" onClick={this.updateOrg} disabled={this.state.isLoading}>Update</Button>
                                            </Form.Group>
                                        </Col>
                                        <Col xs={8}>
                                        </Col>
