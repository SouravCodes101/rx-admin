...
class OfflineOrdersCmp extends React.Component {
    ...
    fetchData=async(id,page)=>{
        let respObj=await CustomerService.fetchCustomerOflineOrders(id,parseInt(page));
        if(respObj===false){
    ...
    pageChange = async (page) => {
        Router.push("/customers/online/storeOrders?page="+parseInt(page)+"&customerId="+this.state.customerId);
    }
    render() {
        return (
            <React.Fragment>
                <div className={"padding-container"}>
                    <Breadcrumb>
                        ...
                        <Breadcrumb.Item active>Store Orders</Breadcrumb.Item>
                    </Breadcrumb>

                    <FlexboxGrid>
                        <FlexboxGrid.Item colspan={5}>
                            <CommonSideNavCmp customerId={this.state.customerId} activeKey={"4"}/>
                        </FlexboxGrid.Item>
                        ...
                        <FlexboxGrid.Item colspan={19}>
                            ...
                            >
                                ...
                                <Column width={150} align="left">
                                    ...
                                    <Cell>{rowData => "₹"+parseFloat(rowData.paidAmount).toFixed(2)}</Cell>
                                </Column>
                                <Column width={150} align="left">
                                    <HeaderCell>Balance Amount</HeaderCell>
                                    <Cell>{rowData => "₹"+parseFloat(rowData.balanceAmount).toFixed(2)}</Cell>
                                </Column>
...
export default OfflineOrdersCmp;
